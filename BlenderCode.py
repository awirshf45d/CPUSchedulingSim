...
def blackboard_reset():
    ...
    # -----------------------------------------------------------------------------------------------------------------
    # clearing the empty objects children end keeping transform
    def clear_parent_keep_transform_recursive(obj):
        for child in list(obj.children):
            clear_parent_keep_transform_recursive(child)
    ...
    parent = bpy.data.objects.get("Black Board Empty")
    if parent:
        clear_parent_keep_transform_recursive(parent)

        # Also clear parent of the empty itself (if it ever had one)
        parent.matrix_world = parent.matrix_world.copy()
        parent.parent = None
